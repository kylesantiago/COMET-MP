# calc calls the math functions 
calc = lambda x,y,z: z(x,y)

# multiply gets the product of the 2 parameters
multiply = lambda x,y: x*y

# divide gets the quotient of the 2 parameters 
...
#setLim makes sure the units and grades are between 0 to 4
def setLim (x):
    while (True):
        if (x >= 0 and x<= 4):
            return x
...
#check UniqueID checks if the ID is unique 
...
#checkUniqueCode checks if the course code is unique 
...
#setID checks if the id is 8 characters
def setID(ID):
    while(True):
...
#checkNAme checks if the name is a string          
def checkName (name):
    while(True):
...
#studentMenu displays the menu for students 
def studentMenu ():
    while(True):
        found = 1
...
#courseMenu displays the menu for courses  
def courseMenu ():
    while(True):
        for x in allCourses:
...
#enrollmentMenu displays the enrollment menu    
def enrollmentMenu ():
    while (True):
        print("\nCourses: ")
        for x in allCourses:
